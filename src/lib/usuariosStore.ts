...
interface FilaDB {
  ...
  asesor_id: string | null; rama_asesor: 'pedagogica' | 'ingles' | null; ejecutivo: string | null; gerencia: string | null; fecha_ingreso: string | null;
...
const aUsuario = (f: FilaDB): Usuario => ({
  ...
  asesorId: f.asesor_id ?? undefined, ramaAsesor: f.rama_asesor ?? undefined, ejecutivo: f.ejecutivo ?? undefined, gerencia: f.gerencia ?? undefined,
...
const aFila = (u: Usuario): FilaDB => ({
  ...
  asesor_id: u.asesorId ?? null, rama_asesor: u.ramaAsesor ?? null, ejecutivo: u.ejecutivo ?? null, gerencia: u.gerencia ?? null,
...
export interface NuevoUsuario {
  ...
  rol: Rol; asesorId?: string; ramaAsesor?: 'pedagogica' | 'ingles'; ejecutivo?: string; gerencia?: string;
}
...
export async function patchUsuario(id: string, patch: Partial<Usuario>): Promise<{ ok: boolean; error?: string }> {
  ...
  if (patch.ramaAsesor !== undefined) fila.rama_asesor = patch.ramaAsesor ?? null;
  if (patch.ejecutivo !== undefined) fila.ejecutivo = patch.ejecutivo ?? null;
  if (patch.gerencia !== undefined) fila.gerencia = patch.gerencia ?? null;
